...
## OpenAI-generated backgrounds (optional)

You can enable AI-generated card backgrounds while keeping the existing overlay/text structure intact.

- Set `USE_OPENAI_IMAGE=true` and provide `OPENAI_API_KEY` in your environment (e.g., `.env.local`).
- When enabled, `lib/imageGenerator.ts` will try to fetch a full-bleed background for the chosen animal via OpenAI (`gpt-image-1`), then apply the same left-side vertical stats overlay and headers.
- If the AI call fails or is disabled, it falls back to the local assets in `public/images/` with no other behavior changes.

Note: Your deployment platform must allow outbound requests to `api.openai.com`.

---

## OpenAI-rendered text (optional)

Enable the model to render the title, left-side stats, and bottom address directly into the image:

- Set `USE_OPENAI_TEXT=true` (and keep `OPENAI_API_KEY` set).
- When enabled, `lib/imageGenerator.ts` will request a full card with text baked in. The output is normalized to 1024x1434. If the call fails, the code falls back to the background-only AI mode (if enabled) or to local images.
- Existing API routes and response formats remain unchanged.
